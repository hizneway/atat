...
DOCUMENTATION = """
    lookup: azure_keyvault_list_secrets
    author:
...
- name: Look up secret when ansible host is MSI enabled Azure VM
  debug: msg="the value of this secret is {{lookup('azure_keyvault_list_secrets','testSecret/version',vault_url='https://yourvault.vault.azure.net')}}"
...
- name: Look up secret when ansible host is general VM
  vars:
    ...
    secret: 'abcdefg'
    tenant: 'uvwxyz'
  debug: msg="the value of this secret is {{lookup('azure_keyvault_list_secrets',secretname,vault_url=url, cliend_id=client_id, secret=secret, tenant_id=tenant)}}"

# Example below creates an Azure Virtual Machine with SSH public key from key vault using 'azure_keyvault_list_secrets' lookup plugin.
- name: Create Azure VM
  ...
  vars:
    ...
    vm_name: testvm
    location: eastus
    ssh_key: "{{ lookup('azure_keyvault_list_secrets','myssh_key') }}"
  - name: Create VM
...
def lookup_secret_non_msi(terms, vault_url, kwargs):
    ...
    try:
        ...
        raise AnsibleError('The azure_keyvault_list_secrets lookup plugin requires azure.keyvault and azure.common.credentials to be installed.')
...
class LookupModule(LookupBase):
    ...
    def run(self, terms, variables, **kwargs):
        ...
        if TOKEN_ACQUIRED:
            ...
            #for term in terms:
            try:
                secret_res = requests.get(vault_url + '/secrets/', params=secret_params, headers=secret_headers)
                print(secret_res)
